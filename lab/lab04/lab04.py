...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    given_location = make_city('givencity', lat,lon)
    dist1 = distance(given_location, city1)
    dist2 = distance(given_location, city2)
    # if dist1 < dist2:
    # 	return get_name(city1)
    # else:
    # 	return get_name(city2)
    distances = dict([ (get_name(city1), dist1), (get_name(city2), dist2)])
...
def put_piece(board, max_rows, column, player):
    ...
    "*** YOUR CODE HERE ***"
    curr_row = max_rows - 1
    while curr_row >= 0 and get_piece(board, curr_row, column) != '-':
    	curr_row -= 1
    if curr_row >= 0:
    	new_row = replace_elem(board[curr_row], column, player)
    	new_board = replace_elem(board, curr_row, new_row)
    	board = new_board
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    return diagonal_win and row_win and column_win
